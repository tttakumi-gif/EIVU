...
opt_perf="-e L1-dcache-load-misses,L1-dcache-loads,L1-dcache-stores,l1d.replacement,LLC-load-misses,LLC-loads,LLC-store-misses,LLC-stores,mem_load_retired.l1_hit,mem_load_retired.l1_miss,mem_load_retired.l2_hit,mem_load_retired.l2_miss,mem_load_retired.l3_hit,mem_load_retired.l3_miss,l2_rqsts.all_demand_data_rd,l2_rqsts.miss,l2_rqsts.references"
...
sudo ~/new-b4-assignment/srv.out --stream=off --process=move --batch=32 &
sleep 0.1
sudo ${perf} ~/new-b4-assignment/build/clt.out --stream=$1 --batch=$batch --process=copy &
sleep 0.5
